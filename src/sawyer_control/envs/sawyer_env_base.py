...
class SawyerEnvBase(gym.Env, Serializable, MultitaskEnv, metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.reset_pos = np.array([ 0.55, -0.05,  0.25471401])
        self.prev_target = None
        self.use_comp = use_compliant_position_controller
        self.count = 0
        self.errors = []
        self.ik_errors = 0
    ...
    def _act(self, action):
        if self.action_mode == 'joint_space_impd':
    ...
    def _position_act(self, action):
        ...
        if self.prev_angles != None:
            self.errors.append(abs(self.prev_target - ee_pos))
        self.count += 1
        if self.count % 1000 == 0:
            print('ik errors: ', self.ik_errors)
